...
export default function Note({ title, content, userId, noteId}) {
    ...
    async function handleSave(event) {
        ...
        try {
            const response = await axios.patch(url + `/edit-note?userid=${userId}&noteid=${noteId}&title=${note.title}&content=${note.content}`);
    ...
    async function handleDelete(event) {
        event.preventDefault()
        try {
            const response = await axios.delete(url + `/delete?userid=${userId}&noteid=${noteId}`);
            console.log(response.data.result);
            setUser(response.data.result)
        } catch (err) {
            console.log(err);
